...
class TrackerController < ApplicationController
    def announce
        ...
        failure 'invalid torrent_pass specified' and return if user.nil?
        failure 'info_hash is missing' and return if params['info_hash'].nil?
        failure 'peer_id is missing' and return if params['peer_id'].nil?
        failure 'port is missing' and return if params['port'].nil?
        failure 'invalid info_hash' and return if params['info_hash'].bytesize != 20
        failure 'invalid peer_id' and return if params['peer_id'].bytesize != 20
